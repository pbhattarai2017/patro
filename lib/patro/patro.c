...
#define MITI_FIRST_YEAR 2000
const char *PATRO_START_BS = "2000-01-01";
const char *PATRO_START_AD = "1943-04-14";

#define MON_1 "Baishakh"
#define MON_2 "Jetha"
#define MON_3 "Asar"
#define MON_4 "Saun"
#define MON_5 "Bhadau"
#define MON_6 "Asoj"
#define MON_7 "Kartik"
#define MON_8 "Mangsir"
#define MON_9 "Pusha"
#define MON_10 "Magh"
#define MON_11 "Falgun"
#define MON_12 "Chaitra"

#define RULE "---------------------------------"

const char *PATRO_weekDays[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};

const char *PATRO_months[] = {
  MON_1,
  MON_2,
  MON_3,
  MON_4,
...
const int title_spaces_for_centering[] = {
  sizeof(RULE) - (sizeof(MON_1) - 1 + 5),
  sizeof(RULE) - (sizeof(MON_2) + 4),
  sizeof(RULE) - (sizeof(MON_3) + 4),
  sizeof(RULE) - (sizeof(MON_4) + 4),
  sizeof(RULE) - (sizeof(MON_5) + 4),
  sizeof(RULE) - (sizeof(MON_6) + 4),
  sizeof(RULE) - (sizeof(MON_7) + 4),
  sizeof(RULE) - (sizeof(MON_8) + 4),
  sizeof(RULE) - (sizeof(MON_9) + 4),
  sizeof(RULE) - (sizeof(MON_10) + 4),
  sizeof(RULE) - (sizeof(MON_11) + 4),
  sizeof(RULE) - (sizeof(MON_12) + 4),
};
...
const int PATRO_YEAR_ARRAY_SIZE = sizeof(PATRO_miti_table) / (sizeof(char) * 12);
const int PATRO_MITI_LAST_YEAR = MITI_FIRST_YEAR + PATRO_YEAR_ARRAY_SIZE - 1;
...
void miti_to_date(struct tm *date, struct miti *miti) {
  ...
  for (int m = 0; m < miti->mon - 1; m++) {
    total_days += PATRO_miti_table[miti->year - MITI_FIRST_YEAR][m];
  }
...
void make_calendar(int mitiYear, int mitiMonth, int mitiDay, int totalDays,
  ...
  // print headline
  for(int i = 0; i < title_spaces_for_centering[mitiMonth - 1] / 2; i ++) {
    printf(" ");
  }
...
int test_miti_validity(struct miti *miti) {
  return miti->year >= MITI_FIRST_YEAR &&
         ...
         miti->day > 0 &&
         miti->day <= PATRO_miti_table[miti->year - MITI_FIRST_YEAR][miti->mon - 1];
}
